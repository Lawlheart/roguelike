...
var GameMap = React.createClass({
  ...
  baddieWalk: function baddieWalk() {
    ...
    for (var i = 0; i < baddies.length; i++) {
      ...
      if (
        next[0] === this.props.player[0] &&
        next[1] === this.props.player[1]
      ) {
  ...
  fogCheck: function fogCheck(x, y) {
    return (
      Math.abs(this.props.player[0] - x) + Math.abs(this.props.player[1] - y) >
        5 ||
  ...
  render: function render() {
    ...
    for (var i = 0; i < tiles; i++) {
      ...
      keyname = "tile-" + i;
      grid.push(<div className={classname} key={keyname} />);
    }
    for (var i = 0; i < this.props.potions.length; i++) {
      potionkey = "potion-" + i;
      potionclass =
        "gear potion x-" +
        this.props.potions[i][0] +
        " y-" +
        this.props.potions[i][1];
    ...
    for (var i = 0; i < this.props.baddies.length; i++) {
      ...
      baddieclass =
        "sprite baddie " +
        this.props.baddieset[i].color +
        " " +
        this.state.baddieFoot +
        " x-";
      baddieclass +=
        this.props.baddies[i][0] + " y-" + this.props.baddies[i][1];
      if (this.fogCheck(this.props.baddies[i][0], this.props.baddies[i][1])) {
        baddieclass += " fog";
      }
      grid.push(
        <div
          className={baddieclass}
          key={baddiekey}
          style={{
    ...
    for (var i = 0; i < this.props.weapons.length; i++) {
      ...
      weaponclass =
        "gear " +
        this.props.weaponset[i] +
        " x-" +
        this.props.weapons[i][0] +
        " y-" +
        this.props.weapons[i][1];
...
var Main = React.createClass({
    ...
    while (boss.length === 0) {
      ...
      for (var i = 0; i < 2; i++) {
        for (var j = 0; j < 3; j++) {
          ...
          console.log(
            this.coordIndex(coords, [location[0] + i, location[1] + j])
          );
      ...
      for (var i = 0; i < invalids.length; i++) {
        for (var j = 0; j < adjacent.length; j++) {
          if (
            adjacent[j][0] == invalids[i][0] &&
            adjacent[j][1] == invalids[i][1]
          ) {
            valid = false;
          }
        }
  ...
  validMove: function validMove(x, y) {
    var onmap =
      x > 0 && x < this.state.gridwidth && y > 0 && y < this.state.gridheight;
  ...
  moveSprite: function moveSprite(e) {
    ...
    if (e.keyCode === 40 || e.keyCode === 83) {
      ...
      if (this.validMove(this.state.player[0], this.state.player[1] + 1)) {
        if (
          bgy < this.state.gridheight - this.state.viewheight &&
          ypos > Math.floor(this.state.viewheight / 2)
        ) {
          bgy += 1;
        } else {
          ypos += 1;
      ...
      if (this.validMove(this.state.player[0] + 1, this.state.player[1])) {
        if (
          bgx < this.state.gridwidth - this.state.viewwidth &&
          xpos > Math.floor(this.state.viewwidth / 2)
        ) {
          bgx += 1;
        } else {
          xpos += 1;
  ...
  resolveMove: function resolveMove(player) {
    ...
    if (
      player[0] === this.state.boss[0][0] &&
      player[1] === this.state.boss[0][1]
    ) {
      if (!this.fightBoss()) {
        return false;
      }
